# Sprint 0 Kapanış Kontrolü

## Tamamlanan

- [x] Mercur yerelde çalıştırıldı.
- [x] PostgreSQL ve Redis servisleri doğrulandı.
- [x] Admin, Vendor ve Storefront ekranları incelendi.
- [x] Seller -> Proposed -> Admin approval -> Storefront akışı denendi.
- [x] Gap Analysis hazırlandı.
- [x] Home, All Listings, Product Details ve Create Listing wireframe'leri hazırlandı.
- [x] Bağımsız uygulama mimarisi ve klasör yapısı kararlaştırıldı.
- [x] README başlangıç dokümanı oluşturuldu.
- [x] Postman collection skeleton oluşturuldu.
- [x] Bağımsız GitHub repository oluşturuldu ve `origin/main` ile bağlandı.
- [x] Wireframe PNG çıktısı `docs/wireframes` klasörüne eklendi.
- [x] Düzenlenebilir Excalidraw kaynak dosyası `docs/wireframes` klasörüne eklendi.

## Kalan

- [ ] Sprint 1-5 GitHub milestone ve issue'ları oluşturulacak.
- [ ] Muhammed ve Zeyad için primary/reviewer sorumlulukları atanacak.
